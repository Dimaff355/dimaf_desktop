...
> **Важно:** Текущая версия является "Proof of Concept" (доказательством концепции). Она демонстрирует установку соединения, передачу изображения и ввод, но не готова для коммерческого использования (отсутствует сжатие видеопотока, нет полноценной архитектуры для работы в режиме системной службы).
...
- **Operator Console:** Консольное приложение (CLI), которое отображает поток (сохраняет кадры), обрабатывает команды оператора и отправляет их хосту.
...
### Шаг 3: Запуск Оператора

Запустите консоль оператора в отдельном терминале:

```bash
dotnet run --project src/OperatorConsole -- ws://localhost:5000/ws <HOST_ID> <PASSWORD>
```

*Замените `<HOST_ID>` на GUID из логов хоста. Пароль опционален, если не задан в конфиге.*

### Управление (Команды CLI)

В консоли оператора доступны следующие команды:

  - `mouse <x> <y>` — переместить мышь (координаты нормализованы от 0 до 1).
  - `click <left|right|middle>` — клик соответствующей кнопкой мыши.
  - `wheel <delta>` — прокрутка колесика.
  - `key <scanCode> <down|up>` — нажатие/отпускание клавиши по скан-коду.
  - `cad` — отправить Ctrl+Alt+Del (Secure Attention Sequence).
    > **Примечание:** Для работы `cad` на хосте должен присутствовать файл `sas.dll` и приложение должно быть запущено с правами администратора. Если библиотека отсутствует, команда будет проигнорирована (см. логи).
  - `monitor_switch <id>` — переключение монитора (если их несколько).
...
## Структура проекта
  ...
  - `src/OperatorConsole` — Клиентская часть (CLI) для теста соединения.
...
### Требует доработки (План развития):
  ...
  - [ ] **GUI Клиент:** Полноценное графическое приложение для оператора вместо консоли.
